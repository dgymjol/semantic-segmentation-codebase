...
class BoundaryLoss(nn.Module):
	...
	def forward(self, preds, embeddings, fsss_gts):
		...
		# # Calculate triplet loss
		# losses = F.relu(pos_similarities.clone() - neg_similarities.clone() + self.margin)
		# loss = losses.sum() / ((losses <= self.epsilon).sum() + self.epsilon)
...
class SupConLoss(nn.Module):
	...
	def forward(self, preds, embeddings, wsss_gts, fsss_gts):
		...
		for cls in s_wsss_cls: # except background class
			...
			# hard_cls = torch.unique(s_pred[new_hard_cls_anchor_mask].type(torch.uint8)).tolist()
			# if hard_cls[0] == 0: hard_cls[1:] # background ignore

			# if len(hard_cls) > self.views:
			# 	hard_cls = random.sample(hard_cls, self.views) # choose n classes randomly

			for cls_anc_embed in cls_anchor_embeds:
				all_neg_similarities = []
				for i in range(self.num_classes-1):
					if i == cls - 1:
						pos_similarity = torch.matmul(cls_anc_embed, self.memory[i].clone().cuda().t()).mean()
					else:
						neg_similarity = torch.matmul(cls_anc_embed, self.memory[i].clone().cuda().t())
						all_neg_similarities.append(neg_similarity)
		
				all_neg_similarities = torch.cat(all_neg_similarities, dim=0)
				neg_similarities.append(all_neg_similarities.mean())
		...
		pos_similarities = torch.stack(pos_similarities, dim=0)
		neg_similarities = torch.stack(neg_similarities, dim=0)

		# Calculate triplet loss
		losses = F.relu(pos_similarities.clone() - neg_similarities.clone() + self.margin).clone()
		loss = losses.mean()
...
def train_net():
	...
	if cfg.GPUS > 1:
		...
		net = nn.parallel.DistributedDataParallel(net, device_ids=[device], output_device=device, find_unused_parameters=True)
